...
if(word(2) == 'DENSITY_NORMAL') then
    mode(2) = 0
else if(word(2) == 'DENSITY_CONSTANT') then
    mode(2) = 1
else
...
do j=1,nz
    za(j) = (j-1) * dz
    exner(j) = merge(1.0 - za(j) / h0, 1.0, mode(1) == 0)
...
if(mode(1) == 0) then
    ...
    call solve_elliptic(max_iter, strategy, strategy_r, psi, coe, f, wksp_O, nr, nz, err, 0)
...
strategy = saved_strategy; strategy_r = saved_strategy_r;
call solve_elliptic(max_iter, strategy, strategy_r, chi, coe, f, wksp_O, nr, nz, err, 0)
print *, "Relaxation uses ", strategy, " steps. Final residue is ", strategy_r, "."
...
do i=1,nr
    do j=1,nz-1
        to_u(i,j) = to_u(i,j)/((sigma(j)+sigma(j+1))/2.0)
    end do
end do
...
do i = 1,nr-1
    do j = 1, nz-1
        ...
        sum_wtheta = sum_wtheta + g0 / theta0 &
           ...
           &          * theta_B(i,j) * r
        sum_q = sum_q + Q_B(i,j) * sigma(j) * r
    end do
end do

print *, "sum_wtheta:", sum_wtheta, ", sum_q:", sum_q
print *, "efficiency(%): ", sum_wtheta / sum_q

cal_eta_avg_wtheta = sum_wtheta / sum_q
end function
